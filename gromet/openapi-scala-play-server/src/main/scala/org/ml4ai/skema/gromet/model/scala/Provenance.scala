...
import org.json4s.{CustomSerializer, DefaultFormats, Formats, JField, JInt, JNull, JObject, JString, JValue, ShortTypeHints}
...
object Provenance {
  ...
  def fromJson(jValue: JValue): Provenance = {
    ...
    val timestampOpt = (jValue \ TIMESTAMP).extractOpt[JValue].map(Model.dateFromJson)
